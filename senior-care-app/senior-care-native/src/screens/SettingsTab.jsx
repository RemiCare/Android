...
function ZoneSettings() {
  ...
  const saveZones = async () => {
    ...
    setSaving(true);
    try {
      // 1. 실제 토큰이 존재할 때만 백엔드 DB에 구역 저장
      if (state.user?.token) {
        const dbRes = await fetch(`${BASE_URL}/api/camera/zone`, {
          method: "POST",
          headers: { 
      ...
      if (res.ok) {
        Alert.alert("저장 완료", state.user?.token
          ? "구역 설정이 데이터베이스와 AI 서버에 완벽히 적용되었습니다."
          : "데모 모드: 구역 설정이 AI 서버에 임시 적용되었습니다.");
        setVisible(false);
      } else {
        Alert.alert("오류", "AI 서버 설정 적용에 실패했습니다.");
      }
    } catch (e) {
      Alert.alert("오류", "저장 실패 — 네트워크 상태 및 서버 실행을 확인하세요.");
    } finally {
      setSaving(false);
    }
  };
